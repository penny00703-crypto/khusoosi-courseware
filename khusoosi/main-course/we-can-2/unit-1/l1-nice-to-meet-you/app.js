/* ================================================================
   We Can 2 · Unit 1 Back to School · L1 (25-min 1v1 paid lesson)
...
const TEACHER_GUIDES = [
  ...
  '<b>Warm-up (2 min cap):</b> Run one WC1 round: "Hello! How are you?" Accept any WC1 answer — "I\'m fine, thank you." is correct here. One round only; do not linger.',
  '<b>Outcome:</b> Point to the We Can 2 cover: same book as school. State the one goal — meet someone NEW, five turns, no reading.',
...
/* ---------- S2 · WC1 warm-up: one old round, then move on ---------- */
addScreen('Warm-up · WC1 round', `
  <div style="display:flex; align-items:center; gap:48px; width:100%; justify-content:center;">
    ...
    <div style="max-width:560px;">
      ...
      <div class="card" style="position:relative; font-size:34px; font-weight:750; color:#17324d; line-height:1.4; margin-top:14px;">
        Hello! <span style="color:#ff735c;">How are you?</span>
        <span class="ar" style="font-size:20px; color:#6e7b8b; margin-top:8px;">مرحبًا! كيف حالك؟</span>
      </div>
      <div style="display:flex; gap:12px; margin-top:16px; flex-wrap:wrap;">
        ${['Hello!','How are you?',"I'm fine, thank you."].map(t=>`
          <span class="pill" style="background:#eef6ff; color:#1670e8; font-size:17px;">${t}</span>`).join('')}
      </div>
      <div style="display:flex; gap:18px; margin-top:22px; align-items:center;">
        <button class="speaker" data-audio="a_t_wc2_leadin.mp3"></button>
        <button id="leadin-done" style="height:56px; padding:0 28px; border:none; border-radius:28px;
          ...
          box-shadow:0 3px 0 #187c50;">I answered!</button>
      </div>
    </div>
  </div>`, el=>({
  onEnter(){
    el.querySelector('#leadin-done').onclick = ()=>{
      sfx('sfx_correct.mp3');
      setTimeout(()=>play('a_t_wc2_reply.mp3'), 350);
      toast('Great! You remember!');
    };
  }
}));
...
addScreen('Today’s mission', `
  <div style="display:flex; gap:44px; align-items:center; width:100%; justify-content:center;">
    <div style="max-width:560px;">
      ...
      <div style="font-size:19px; color:#6e7b8b; margin-bottom:18px;">By the end, you can talk to a new friend — five turns, no reading.</div>
      ${[['a_t_goal1.mp3','Say <b style="color:#ff8c42;">"It\'s nice to meet you"</b>','قُل: سعيد بلقائك'],
         ['a_t_goal2.mp3','Answer <b style="color:#2b6cb0;">"I\'m great, thanks"</b>','قُل: أنا بخير، شكرًا'],
         ['a_t_goal3.mp3','Say <b style="color:#1f9d6c;">"I\'m first!"</b> your line number','قُل رقمك في الصف']].map(([au,en,ar])=>`
    ...
    <div style="text-align:center;">
      ...
      <div style="margin-top:14px; font-size:19px; font-weight:700; color:#2b6cb0;">We Can 2 · Same as your school!</div>
...
addScreen('Listen · two new friends', `
  <div style="text-align:center; width:100%;">
    ...
    <div style="display:flex; align-items:center; justify-content:center; gap:18px; margin-top:26px;">
      ...
      <div style="text-align:left;">
        <div style="font-size:24px; font-weight:750; color:#17324d;">Listen. Point to who is talking.</div>
    ...
    <div class="teacher-only" style="margin-top:14px; justify-content:center;">
      <span class="pill" style="background:#fff3e0; color:#d96f2a; font-size:14px;">♪ Official audio pending · CD1 03 · teacher voice for now</span>
    </div>
  </div>`);
...
addScreen('Stage habits', `
  <div style="text-align:center; width:100%;">
    ...
    <h1 style="font-size:38px; color:#17324d; margin:12px 0 4px;">Talk like a star</h1>
    ...
    <div style="display:flex; gap:22px; justify-content:center;">
      ${[
        ['#ffc84a','Big smiles','ابتسامة كبيرة','<circle cx="32" cy="32" r="26" fill="#fff"/><circle cx="23" cy="26" r="4" fill="#17324d"/><circle cx="41" cy="26" r="4" fill="#17324d"/><path d="M20 38 Q32 50 44 38" stroke="#17324d" stroke-width="4" fill="none" stroke-linecap="round"/>'],
        ['#1670e8','Gestures','حركات','<rect x="22" y="26" width="20" height="24" rx="9" fill="#fff"/><rect x="20" y="10" width="6" height="20" rx="3" fill="#fff"/><rect x="29" y="7" width="6" height="23" rx="3" fill="#fff"/><rect x="38" y="10" width="6" height="20" rx="3" fill="#fff"/><path d="M50 18 Q58 12 54 6" stroke="#fff" stroke-width="3" fill="none" stroke-linecap="round"/>'],
        ['#ff735c','Strong voice','صوت قوي','<path d="M12 26 L30 18 L30 46 L12 38 Z" fill="#fff"/><path d="M36 24 Q44 32 36 40" stroke="#fff" stroke-width="4" fill="none" stroke-linecap="round"/><path d="M42 18 Q54 32 42 46" stroke="#fff" stroke-width="4" fill="none" stroke-linecap="round"/>'],
        ['#24a66a','Eye contact','تواصل بالعيون','<ellipse cx="20" cy="32" rx="12" ry="9" fill="#fff"/><circle cx="20" cy="32" r="4.5" fill="#17324d"/><ellipse cx="46" cy="32" rx="12" ry="9" fill="#fff"/><circle cx="46" cy="32" r="4.5" fill="#17324d"/>']
      ].map(([c,en,ar,svg])=>`
        <div class="card" style="width:240px; padding:22px 14px; text-align:center;">
          <div style="width:92px; height:92px; margin:0 auto 14px; border-radius:50%; background:${c};
            display:flex; align-items:center; justify-content:center; box-shadow:0 8px 18px rgba(23,50,77,.16);">
            <svg viewBox="0 0 64 64" width="58" height="58">${svg}</svg>
          </div>
          <div style="font-size:22px; font-weight:800; color:#17324d;">${en}</div>
...
addScreen('Ordinals · first to fifth', `
  <div style="display:flex; gap:36px; width:100%; align-items:center; justify-content:center;">
    <div style="width:470px; position:relative;">
      ...
      <div style="position:absolute; top:-12px; left:-12px; background:#2b6cb0; color:#fff; font-weight:800;
    ...
    <div style="width:560px;">
      <div style="display:flex; align-items:center; gap:14px; margin-bottom:14px;">
        ...
        <div style="font-size:22px; font-weight:750; color:#17324d;">Line up! Say your place.</div>
      </div>
      ${[['first','#ff735c'],['second','#ff8c42'],['third','#1670e8'],['fourth','#24a66a'],['fifth','#8e44ad']].map(([w,c],k)=>`
        <div class="card" data-audio="a_t_${w}.mp3" style="display:flex; align-items:center; gap:16px; padding:9px 18px;
          ...
          <span style="font-size:27px; font-weight:800; color:#2b3a55;">I'm <b style="color:${c};">${w}</b>!</span>
        </div>`).join('')}
    </div>
  </div>`);
...
addScreen('Greeting Line practice', `
  <div style="text-align:center; width:100%;">
    ...
    <div style="font-size:24px; font-weight:750; color:#17324d; margin:12px 0 4px;">Point → say the place → tap to check</div>
    ...
    <div style="display:flex; gap:18px; justify-content:center; align-items:flex-end;">
      ${[['sec_teacher.png','first','#ff735c'],['sec_char_noura.png','second','#ff8c42'],
         ['sec_char_cat.png','third','#1670e8'],['sec_char_wolf.png','fourth','#24a66a'],[null,'fifth','#8e44ad']]
        .map(([img,w,c])=>`
          ...
          <div style="height:230px; display:flex; align-items:flex-end; justify-content:center;">
            ${img
              ...
              : `<div style="width:130px; height:130px; border-radius:50%; background:#fff;
                   border:4px dashed #8e44ad; display:flex; align-items:center; justify-content:center;
                   font-size:30px; font-weight:800; color:#8e44ad;">YOU</div>`}
          </div>
...
addScreen('Ordinal flash check', `
  <div style="text-align:center; width:100%;">
    ...
    <div style="display:flex; gap:40px; justify-content:center; align-items:center; margin-top:18px;">
      <div class="card" style="width:340px; height:340px; display:flex; flex-direction:column; align-items:center; justify-content:center;">
        <div id="fc-num" style="font-size:130px; font-weight:800; color:#2b6cb0; line-height:1;">?</div>
      ...
      <div style="width:380px;">
        <div id="fc-answer" class="card" style="min-height:110px; display:flex; align-items:center; justify-content:center;
          font-size:32px; font-weight:800; color:#17324d;">Say it first!</div>
        <div style="display:flex; gap:14px; margin-top:18px; justify-content:center;">
          <button id="fc-reveal" style="height:52px; padding:0 30px; border:none; border-radius:26px;
            background:#ff8c42; color:#fff; font-size:20px; font-weight:750; cursor:pointer;
            box-shadow:0 3px 0 #d96f2a;">Reveal</button>
          <button id="fc-next" style="height:52px; padding:0 30px; border:none; border-radius:26px;
            background:#2b6cb0; color:#fff; font-size:20px; font-weight:750; cursor:pointer;
            box-shadow:0 3px 0 #1d4e82;">Next card</button>
        </div>
  ...
  el.querySelector('#fc-reveal').onclick=()=>{
    if (di>=5) return;
    const w=ORD[deck[di]];
    el.querySelector('#fc-answer').innerHTML='I\'m <b style="color:#1670e8;">'+w+'</b>!';
    play('a_t_'+w+'.mp3');
  };
  ...
  <div style="display:flex; gap:34px; width:100%; justify-content:center; align-items:stretch;">
    <div style="width:640px; display:flex; flex-direction:column;">
      <div style="display:flex; align-items:center; gap:16px; margin-bottom:12px;">
        ...
        <span id="ev-round" class="card" style="padding:6px 16px; font-size:17px; font-weight:800; color:#ff735c;">Friend 1 / 3</span>
      </div>
    ...
    <div style="width:300px;">
      <div class="card" style="height:100%; min-height:440px; position:relative; overflow:hidden; padding:14px;
        background:linear-gradient(180deg,#dfeaf5,#c3d6ea);">
        <div style="text-align:center; font-size:18px; font-weight:800; color:#17324d; margin-bottom:10px;">FRIENDS ABOARD</div>
        ${[3,2,1].map(f=>`
          <div style="display:flex; align-items:center; gap:10px; margin-bottom:10px;">
            <span style="width:38px; height:38px; border-radius:10px; background:#17324d; color:#fff;
            ...
            <div id="ev-slot${f}" style="flex:1; height:56px; border-radius:12px; background:#ffffff90;
              border:2px dashed #9fb3c8; display:flex; align-items:center; justify-content:center;
        ...
        <div id="ev-win" style="position:absolute; inset:0; display:none; flex-direction:column; align-items:center;
          ...
          <div style="font-size:34px; font-weight:800; color:#17324d;">All friends aboard!</div>
    ...
    el.querySelector('#ev-lines').innerHTML = LINES.map((l,k)=>`
      <div class="card ev-line" data-k="${k}" style="display:flex; align-items:center; gap:10px; padding:8px 14px;
        ...
        ${k<step ? 'background:#e8f8f0; border:2px solid #24a66a; opacity:.75;' :
          k===step ? 'border:3px solid #ff8c42; box-shadow:0 8px 20px rgba(255,140,66,.25);' : 'opacity:.55;'}">
        <span style="min-width:64px; font-size:13px; font-weight:800; letter-spacing:.5px;
          color:${l.who==='friend'?'#1670e8':'#e8443a'};">${l.who==='friend'?'FRIEND':'YOU'}</span>
        <button class="speaker small" data-audio="${l.au}" style="width:38px;height:38px;background-size:20px;"></button>
        <span style="font-size:19px; font-weight:700; color:#2b3a55; flex:1;">${l.t}</span>
  ...
  return {
    onEnter(){
      ...
      [1,2,3].forEach(f=>{ const s=el.querySelector('#ev-slot'+f);
        s.innerHTML='waiting…'; s.style.border='2px dashed #9fb3c8'; });
      render();
    }
  };
});
  ...
  function render(){
    host.innerHTML = lines.map((t,k)=>`
      <div class="card dp-c" data-k="${k}" style="display:flex; align-items:center; gap:16px; padding:12px 22px;
        ...
        <button class="speaker small" data-audio="${audios[k]}"></button>
        <span style="font-size:25px; font-weight:700; color:#2b3a55; flex:1;">${t}</span>
...
addScreen('Check 1 · Real talk', `
  <div style="display:grid; grid-template-columns:430px 1fr; gap:54px; align-items:center; width:100%; max-width:1080px;">
    ...
    <div>
      ...
      <h1 style="font-size:42px; color:#17324d; margin:16px 0 8px;">Meet your new friend</h1>
...
addScreen('Repair · Check 2', `
  <div style="display:grid; grid-template-columns:410px 1fr; gap:48px; align-items:center; width:100%; max-width:1080px;">
    ...
    <div>
      ...
      <h1 style="font-size:40px; color:#17324d; margin:14px 0 12px;">Try with a new friend</h1>
      <div id="repair-view" class="card" style="min-height:170px; display:flex; align-items:center; justify-content:center;
        text-align:center; font-size:27px; font-weight:800; color:#17324d; line-height:1.5; padding:16px 22px;">
  ...
  const steps=[
    ...
    ()=>{ view.innerHTML='<div style="font-size:22px;">Watch me. Then you.<br><span style="color:#1670e8;">Hi! It\'s nice to meet you. … I\'m great, thanks. And you?</span></div>'; play('a_t_tt_all.mp3'); },
...
addScreen('School practice', `
  <div style="display:flex; gap:40px; width:100%; justify-content:center;">
    ...
    <div style="width:520px;">
      <div style="font-size:22px; font-weight:700; color:#2b3a55; margin-bottom:4px; text-align:center;">2 · Which place do you hear?</div>
  ...
  function renderChoose(){
    ...
    el.querySelector('#ex-opts').innerHTML=opts.map(([t,ok])=>`
      <div class="card ex-o" data-ok="${ok}" style="padding:14px 20px; margin-bottom:12px; cursor:pointer;
        font-size:21px; font-weight:700; color:#2b3a55; text-align:center;">${t}</div>`).join('');
  ...
  el.querySelector('#ex-opts2').innerHTML=opts2.map(([t,ok])=>`
    <div class="card ex-o2" data-ok="${ok}" style="padding:14px 20px; margin-bottom:12px; cursor:pointer;
      font-size:23px; font-weight:800; color:#2b3a55; text-align:center;">${t}</div>`).join('');
...
addScreen('Learning record', `
  <div style="text-align:center;">
    ...
    <div style="display:flex; gap:30px; justify-content:center;">
      ${[['Meet a new friend','a_t_tt1.mp3','#ff735c'],['Say "I\'m great, thanks"','a_t_tt4.mp3','#1670e8'],['Say my line number','a_t_first.mp3','#24a66a']]
        .map(([t,au,c])=>`
        <div class="badge" data-audio="${au}" style="cursor:pointer; width:280px; padding:26px 18px; border-radius:22px;
          ...
          <div style="font-size:22px; font-weight:700; color:#2b3a55;">${t}</div>
        </div>`).join('')}
    </div>
    <div style="margin-top:18px;">
      <span class="pill" style="background:#fff3e0; color:#d96f2a; font-size:17px;">Badge: First Friend in G2</span>
    </div>
    <div id="ability-level" class="card" style="display:inline-flex; margin-top:16px; padding:10px 20px; font-size:19px; font-weight:800; color:#17324d;">Level: not recorded</div>
  </div>`, el=>({
  onEnter(){
...
addScreen('Exit task · Home review', `
  <div style="display:flex; gap:40px; width:100%; justify-content:center; align-items:stretch;">
    <div class="card" style="width:520px; padding:26px 30px; text-align:center;">
      <div style="font-size:26px; font-weight:750; color:#17324d; margin-bottom:6px;">Show me your line number!</div>
      ...
      <div style="display:flex; gap:8px; justify-content:center; margin-bottom:14px;">
        ${['1st','2nd','3rd','4th','5th'].map((p,k)=>`
          <button class="exit-pos" data-w="${['first','second','third','fourth','fifth'][k]}" style="width:56px; height:56px;
            border:none; border-radius:14px; background:#1670e8; color:#fff; font-size:17px; font-weight:800;
            cursor:pointer; box-shadow:0 3px 0 #0e56b9;">${p}</button>`).join('')}
      </div>
      <div id="exit-cue" style="min-height:42px; font-size:26px; font-weight:800; color:#ff735c;">Teacher picks a place — you say it!</div>
    ...
    <div class="card" style="width:520px; padding:26px 30px; text-align:center;">
      ...
      <div style="background:#fdf6ec; border:2px dashed #d9c9a3; border-radius:16px; padding:14px 18px;">
        <div style="font-size:21px; font-weight:750; color:#17324d;">Home review · four stations</div>
  ...
  el.querySelectorAll('.exit-pos').forEach(b=> b.onclick=()=>{
    el.querySelectorAll('.exit-pos').forEach(x=>x.style.background='#1670e8');
    b.style.background='#ff735c';
