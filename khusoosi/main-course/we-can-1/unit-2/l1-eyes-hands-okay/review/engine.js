...
(function(){
  ...
  const visual=a=>`<span class="action-thumb" style="background-image:url('${A+a.img}');background-position:${a.pos||'center'};background-size:${a.size||'contain'}"></span>`;
  ...
  // Station 1: every target is seen in full, heard, repeated and marked.
  ...
  // Station 2: the prompt contains no readable answer and the options are image-only.
  let listenIndex=0;
  ...
  // Station 3: one complete speaking target at a time; hint and model are separate.
  ...
  function renderSpeak(){
    ...
    document.getElementById('speak-img').src=A+a.img;
    document.getElementById('speak-img').alt=a.en;
    document.getElementById('speak-answer').textContent='Say it first.';
    ...
    document.getElementById('speak-hint').onclick=()=>{document.getElementById('speak-answer').textContent=a.hint||a.en.split(' ')[0]+'…';};
  ...
  // Station 4: a picture is the question; the learner reads text options without audio.
  let examIndex=0;
  function renderExam(){
    ...
    const picture=document.getElementById('st4-picture');picture.src=A+target.img;picture.alt=target.en;
